...
# - heartbeat: touches logs/watchdog.heartbeat every cycle
# - batch alive test: logs/scaleout.log mtime fresh (trainer writes constantly)
...
LOCK=logs/scaleout.launch.lock
note() { echo "[$(date '+%F %T')] $*" >> "$WLOG"; }
note "watchdog started (pid $$)"
relaunches=0
while true; do
    touch "$HB"
    if grep -q "SCALEOUT BATCH FINISHED" logs/scaleout.log 2>/dev/null; then
        note "batch finished — watchdog exiting"
        exit 0
    fi
    age=$(( $(date +%s) - $(stat -c %Y logs/scaleout.log 2>/dev/null || echo 0) ))
    if [ "$age" -gt 1200 ]; then
