...
Usage: 20b_render_section.py <section>   (GPS-retrain variant: colmap_db/<sec>/gps/gs_scene, splat_output/<sec>_gps)
"""
...
sec = sys.argv[1]
ck = torch.load(f'splat_output/{sec}_gps/ckpts/ckpt_29999_rank0.pt', map_location='cuda', weights_only=False)
sp = ck['splats']
means, quats = sp['means'], sp['quats']
scales, opac = torch.exp(sp['scales']), torch.sigmoid(sp['opacities'])
colors = torch.cat([sp['sh0'], sp['shN']], 1)
print(f'[{sec}_gps] gaussians: {len(means)}')
parser = Parser(f'colmap_db/{sec}/gps/gs_scene', factor=1, normalize=True, test_every=8)
os.makedirs(f'splat_output/{sec}_gps/test_renders', exist_ok=True)
    ...
    for k, i in enumerate(idxs):
        ...
        if k == 4:
            ...
            Image.fromarray(pair).save(
                f'splat_output/{sec}_gps/test_renders/{split}_mid_psnr{psnrs[-1]:.1f}.jpg', quality=90)
    print(f'[{sec}_gps] {split}: PSNR p50={np.median(psnrs):.2f}')
print(f'[{sec}_gps] RENDER TEST DONE')
