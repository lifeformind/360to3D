#!/usr/bin/env python3
"""Rigidly align a section's LingBot trajectory to its route window and
...
Usage: 19_align_section.py <section> <route_frac_a> <route_frac_b> <cam_height_m>
"""
...
def main(sec, fa, fb, cam_h):
    fa, fb, cam_h = float(fa), float(fb), float(cam_h)
    ...
    # route window in metres (map y flipped)
    d = json.load(open('colmap_db/amanorthv/route_polyline.json'))
    m_per_px = 1.5
    Rpts = np.array(d['points_px']) * np.array([1.0, -1.0]) * m_per_px
    rs = np.concatenate([[0], np.cumsum(np.linalg.norm(np.diff(Rpts, axis=0), axis=1))])
    sa, sb = fa * rs[-1], fb * rs[-1]
    su = np.linspace(sa, sb, N)
    W = np.stack([np.interp(su, rs, Rpts[:, 0]), np.interp(su, rs, Rpts[:, 1])], 1)

    # arc-fraction correspondence + similarity Procrustes (rotation only, no reflection)
    seg = np.linalg.norm(np.diff(P2, axis=0), axis=1)
    s_arc = np.concatenate([[0], np.cumsum(seg)])
    s_arc /= max(s_arc[-1], 1e-9)
    Wm = np.stack([np.interp(s_arc, np.linspace(0, 1, N), W[:, 0]),
                   np.interp(s_arc, np.linspace(0, 1, N), W[:, 1])], 1)
    ma, mb = P2.mean(0), Wm.mean(0)
    A, B = P2 - ma, Wm - mb
    H = A.T @ B
    U, Sv, Vh = np.linalg.svd(H)
    D = np.eye(2); D[1, 1] = np.sign(np.linalg.det(Vh.T @ U.T))
    Rot = Vh.T @ D @ U.T
    scl = (Sv * np.diag(D)).sum() / max((A ** 2).sum(), 1e-9)
    P_new = (scl * (Rot @ A.T)).T + mb
    rms = float(np.sqrt(((P_new - Wm) ** 2).sum(1).mean()))
    ...
    print(f"[gate] rms to route window = {rms:.2f} m  (want < ~8)")
    ...
    for link, target in [(f'{gs}/images', os.path.abspath(f'cubefaces_rgba/{sec}')),
                         (f'{gs}/sparse', os.path.abspath(f'colmap_db/{sec}/sparse'))]:
        if os.path.islink(link):
            os.remove(link)
        os.symlink(target, link)
    np.savez(f'colmap_db/{sec}/poses_aligned.npz', c2w=c2w, kappa=kap, scale=scl)
